...
```powershell # 1. Fire a live checkout transaction payload through the API Gateway Edge $loginResponse = Invoke-RestMethod -Uri *[http://localhost:**3000**/api/users/login*](http://localhost:**3000**/api/users/login*) -Method **POST** -Body '{*email*: *[terminaluser@example.com](mailto:terminaluser@example.com)*, *password*: *securepass123*}' -ContentType *application/json" $token = $loginResponse.token $cartBody = @{productId=1; quantity=1} | ConvertTo-Json Invoke-RestMethod -Uri *[http://localhost:**3000**/api/cart/add*](http://localhost:**3000**/api/cart/add*) -Method **POST** -Body $cartBody -ContentType *application/json* -Headers @{Authorization = *Bearer $token*}

Invoke-RestMethod -Uri *[http://localhost:**3000**/api/orders/checkout*](http://localhost:**3000**/api/orders/checkout*) -Method **POST** -Headers @{Authorization = *Bearer $token*} | ConvertTo-Json
...
- Ensure you have [Docker Desktop](https://[www.docker.com/products/docker-desktop/](https://www.docker.com/products/docker-desktop/)) installed and actively running.
...
| --- | --- | --- |
| **API Gateway Core** | `[http://localhost:3000`](http://localhost:3000`) | Synchronous Express REST Router Endpoints |
| **Grafana UI Dashboard** | `[http://localhost:3001`](http://localhost:3001`) | Visual Time-Series Metric Dashboards |
| **Jaeger Tracing Interface** | `[http://localhost:16686`](http://localhost:16686`) | Distributed Tracing Latency Timelines |
| **Prometheus Telemetry Engine** | `[http://localhost:9090`](http://localhost:9090`) | Time-Series Target Scraping Health Status |
| **RabbitMQ Management Dashboard** | `[http://localhost:15672`](http://localhost:15672`) | Real-time Message Broker Queue Monitors |
| **Saga Worker Telemetry Channel** | `[http://localhost:3002/metrics`](http://localhost:3002/metrics`) | Raw Asynchronous Worker Metric Streams |
